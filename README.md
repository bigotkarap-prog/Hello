...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Ponyo-style watercolor ocean background */
        .ocean-bg {
            position: fixed;
            top: 0;
            left: 0;
        ...
        /* Clouds - Ponyo style soft and puffy */
        .cloud {
            position: absolute;
        ...
        .cloud-1::after { width: 80px; height: 50px; top: -20px; right: 10px; }
        ...
        .cloud-2::after { width: 70px; height: 40px; top: -15px; right: 15px; }
        ...
        .cloud-3::after { width: 90px; height: 55px; top: -25px; right: 20px; }
        ...
        /* Opening overlay - translucent so ocean is visible behind */
        .opening-overlay {
            position: fixed;
            top: 0;
            left: 0;
        ...
        .opening-bubble {
            width: 200px;
            height: 200px;
            background: radial-gradient(circle at 30% 30%, rgba(255,255,255,0.9) 0%, rgba(135,206,235,0.6) 40%, rgba(72,209,204,0.4) 100%);
            border-radius: 50%;
        ...
        .ponyo-flame {
            width: 16px;
            height: 24px;
            background: radial-gradient(ellipse at bottom, #FF4757 0%, #FFA502 40%, #FFDD59 70%, transparent 100%);
        ...
        @keyframes nameGlow {
            0%, 100% { text-shadow: 0 0 20px rgba(255,107,107,0.4), 3px 3px 0px rgba(0,0,0,0.1); }
            50% { text-shadow: 0 0 40px rgba(255,107,107,0.7), 0 0 60px rgba(255,182,193,0.4), 3px 3px 0px rgba(0,0,0,0.1); }
        }
        ...
        .bubble {
            position: absolute;
            border-radius: 50%;
            background: radial-gradient(circle at 30% 30%, rgba(255,255,255,0.9) 0%, rgba(135,206,235,0.4) 40%, rgba(72,209,204,0.2) 100%);
        ...
        .secret-toast {
            ...
            box-shadow: 0 6px 24px rgba(255,107,107,0.35), 3px 3px 0px rgba(255,182,193,0.4);
            z-index: 9999;
            opacity: 0;
        ...
        .envelope-overlay {
            ...
            /* Soft blush radial background matching reference */
            background:
                radial-gradient(ellipse 80% 70% at 50% 40%, #f9ebe8 0%, #f2d8d2 55%, #e8c5bc 100%);
            z-index: 2000;
            display: flex;
            flex-direction: column;
            justify-content: center;
        ...
        /* Outer wrapper — perspective lives here */
        .envelope-wrapper {
            position: relative;
            width: 320px;
            height: 230px;
        ...
        /* Inner fold shadow lines — left, right, bottom triangles */
        .env-body::before {
            content: '';
            position: absolute;
            inset: 0;
        ...
        .env-letter-inner {
            ...
            transition: opacity 0.55s 1.2s ease, transform 0.55s 1.2s cubic-bezier(0.34,1.2,0.64,1);
        }
        ...
        .env-letter-emoji { font-size: 2.2rem; line-height: 1; }
        ...
        .env-flap-container {
            ...
            z-index: 6;
            pointer-events: none;
            /* perspective on the container so child rotates correctly */
            perspective: 1200px;
            perspective-origin: 50% 0%;
        }
        ...
        .env-flap {
            ...
            top: 0; left: 0; right: 0;
            height: 130px;
            /* Blush gradient matching back of flap */
            background: linear-gradient(175deg, #f5d5ce 0%, #eec5bc 50%, #e8b8ae 100%);
            border-radius: 14px 14px 0 0;
            /* V-shape cut — the downward-pointing triangle of a real envelope flap */
        ...
        /* Highlight stripe along the top edge of flap */
        .env-flap::before {
            content: '';
            position: absolute;
            top: 0; left: 0; right: 0;
        ...
        /* Soft shadow that deepens as flap lifts */
        .env-flap::after {
            content: '';
            position: absolute;
            inset: 0;
        ...
        /* ── Flower seal at the V-point ── */
        .env-seal {
            position: absolute;
            /* sits at the tip of the V flap — vertically at ~78% of flap height (130px) */
            ...
            z-index: 7;
            pointer-events: none;
            transition: opacity 0.3s 0.05s ease, transform 0.3s 0.05s cubic-bezier(0.34,1.2,0.64,1);
        ...
        /* hide old hint; label replaces both */
        .envelope-click-hint { display: none; }

        @keyframes bouquetRain {
...
(&gt;🌷</div>
        ...
        <div class="message-card">
            <div class="message-text">Wishing you a day as bright and wonderful as you are — may this year bring you joy, laughter, and all the little things that make you smile.</div>
        </div>
    ...
    <script>
        ...
        function createWaterBalloon() {
            ...
            balloon.style.background = `radial-gradient(circle at 35% 35%,
        ...
        function shakeBouquet() {
            ...
            for (let i = 0; i < 12; i++) {
                setTimeout(() => {
                    createMagicParticle(rect.left + rect.width/2, rect.top + rect.height/2);
                }, i * 50);
            }
        }

        function createBouquetItem() {
            ...
            el.style.cssText = `
                ...
                z-index: 998;
                pointer-events: none;
                animation: bouquetRain ${2.2 + Math.random() * 2.2}s ease-in forwards;
            `;
            el.textContent = '💐';
        ...
        function celebrate() {
            for (let i = 0; i < 35; i++) setTimeout(createBouquetItem, i * 55);
        ...
        function startMainScene() {
            ...
            setInterval(createWaterBalloon, 3000);
        ...
        function openEnvelope() {
            ...
            // Capture current translateY so the settle animation starts from where it is
            const bobY = bob.getBoundingClientRect().top - bob.parentElement.getBoundingClientRect().top;
            bob.style.setProperty('--bob-offset', bobY + 'px');

            // Swap to settle animation (glides to rest), then hold still
            bob.classList.add('open');

            // Open the envelope flap + letter after bob settles
            setTimeout(() => {
        ...
        // Opening overlay click handlers
